...
- **多幣別資產換算** — SHOULD (P1)：USD ↔ NTD 即時匯率
- **投資組合再平衡建議** — SHOULD (P1)：基於風險屬性
- **稅務計算** — SHOULD (P1)：台灣股利、美國 30% 預扣稅
- **自動券商 API 串接** — MAY (P2)：待台灣券商開放 API

### 3.3 v3（roadmap — P2 優先級)
...
| 優先級 | 類別 | 需求 | 對應 AC |
|---|---|---|---|
| **P0** | MUST | 使用者可以用 email + 密碼註冊帳號 | AC-001, AC-002, AC-003 |
| **P0** | MUST | 使用者可以新增/編輯/刪除持股 | AC-004, AC-005, AC-006 |
| **P0** | MUST | 統一 dashboard 顯示總資產 + 配置 | AC-007, AC-008 |
| **P0** | MUST | 月底自動建立資產快照 | AC-009, AC-010 |
| **P1** | SHOULD | 多幣別資產換算（USD ↔ NTD） | v2.0 |
| **P1** | SHOULD | 投資組合再平衡建議 | v2.0 |
| **P1** | SHOULD | 稅務計算（台灣股利 + 美國預扣稅） | v2.0 |
| **P2** | MAY | 多券商 API 自動串接 | v3.0 |
| **P2** | MAY | 加密錢包整合 | v3.0 |
| **P2** | MAY | 團隊共用帳戶 | v3.0 |
...
### 10.1.1 ⭐ Competitive Quadrant Chart（MetaGPT 強制 — 從我們看源碼學來）
...
quadrantChart
    ...
    x-axis "低價格" --> "高價格"
    y-axis "低台股支援" --> "高台股支援"
    quadrant-1 "明星：台灣本土且免費"
    quadrant-2 "小眾但台灣友善"
    quadrant-3 "歐美工具（不適合台灣）"
    quadrant-4 "高價但功能強"
    "Personal Capital": [0.7, 0.1]
    "Money Pro": [0.8, 0.5]
    "Excel 自製": [0.0, 0.3]
    "試算表範本": [0.0, 0.2]
    "銀行 App（個別）": [0.0, 0.7]
    "Wealth Dashboard (我們)": [0.0, 0.9]
```

**象限解讀**：
- quadrant-1 (低價 + 高台股支援)：**明星區** — 我們的目標
- quadrant-4 (高價 + 高台股支援)：商用客戶（如 Money Pro）
- quadrant-3 (高價 + 低台股支援)：**死區**（不適合台灣）
- quadrant-2 (低價 + 低台股支援)：Excel 自製族

### 10.1.2 ⭐ Open Questions / Anything UNCLEAR（MetaGPT 標準 — 誠實列出還沒解決的問題）
...
1. **TWSE API 限制**：即時報價是否需付費？月費多少？
2. **法律合規**：跨幣別資產顯示需要金管會許可嗎？
3. **技術債務**：Vite SPA 架構能否撐到 10 萬用戶？
4. **團隊**：需要前端 + 後端 + DevOps 共幾人？
5. **時程**：客戶希望 MVP 幾月上線？
6. **資料來源**：Yahoo Finance / TWSE / CoinGecko 的 SLA 與可用率？
7. **備援**：報價 API 全部掛掉時怎麼辦？（已用降級機制，但完整失效 7 天怎辦）

### 假設（需 Sean 確認）

| 假設 | 影響 | 風險 |
|---|---|---|
| 使用者願意每月付 NT$299 | 定價模式 | 30% 機率不願意 |
| v1 只做繁中（不做英文） | 開發時間 | 鎖定族群 |
| 用 Next.js 16 + Prisma | 技術棧 | 學習曲線 |
| Stripe 處理訂閱 | 金流 | 2.9% 手續費 |
| 純前端報價抓取（不後端 proxy） | 簡單但 CORS 風險 | Yahoo Finance 可能擋 |

### 需要的外部輸入

- [ ] Sean：最終定價決策（NT$299 vs NT$499 vs NT$999）
- [ ] Sean：是否做英文版
- [ ] Alan (CTO)：資料庫選擇（PostgreSQL vs SQLite 起步）
- [ ] Sean：MVP 上線時程
- [ ] Sean：是否需要銀行 API 自動串接
```
...
*驗證分數：見 `scripts/validate_prd.py` 輸出*## 15. 深度市調報告 (Deep Market Research)

### 15.1 市場規模

**全球資產管理 / 個人理財平台市場（2025）**
- 規模：**US$15 億**（2025）→ 預估 **US$34 億**（2030），CAGR 17.8%
- 主要廠商：Mint / Personal Capital (Empower) / YNAB / Monarch Money / Rocket Money
...
- 加密貨幣持有者：**300 萬人**
- 基金 / ETF 投資人：**200 萬人**
- 高資產族群（>NT$1,000 萬）：**50 萬戶**
- 理財規劃需求：**100 萬人 / 年**

**目標細分**
- 個人免費（NT$0）：500 萬 × 3% 採用 → 15 萬活躍（無直接收入）
- 個人 Pro（NT$99/月）：500 萬 × 2% 採用 × NT$99 × 12 月 = **NT$118.8 億 ARR** 潛在
- 加密貨幣版（NT$149/月）：300 萬 × 1% 採用 × NT$149 × 12 月 = **NT$53.6 億 ARR** 潛在
- 高資產版（NT$999/月）：50 萬 × 3% 採用 × NT$999 × 12 月 = **NT$179.8 億 ARR** 潛在
- 企業財富管理（NT$9,999/月）：5,000 × 30% × NT$9,999 × 12 月 = **NT$179.9 億 ARR** 潛在
- **合計總潛在 ARR**：**NT$532.1 億**

### 15.2 競品分析

| 競品 | 公司 | 價格 | 強項 | 弱項 |
|---|---|---|---|---|
| **Mint** | Intuit（美） | 免費 | 美國市佔大 | 偏美國金融、台灣無用 |
| **Personal Capital** | Empower（美） | 免費 | 高資產理財 | 偏美國、繁中介面無 |
| **YNAB** | YNAB（美） | US$14.99/月 | 預算管理強 | 英文、偏預算非資產 |
| **Monarch Money** | Monarch（美） | US$99/年 | 現代 UI、訂閱制 | 英文 |
| **MoneyDJ / 玩股網** | 玩股網（台） | 免費 + 付費 | 台股資訊強 | 偏股票、無加密貨幣整合 |
| **基富通 / 鉅亨網** | 各家台灣（台） | 免費 | 基金 / ETF | 偏單一資產類別、無整合 |
| **Wealth Dashboard（本專案）** | Sean Li（台） | NT$0-9,999/月 | 整合台股 + 加密貨幣 + 基金 + ETF + 純前端 + 繁中友善 | 規模小、無法即時報價 |

```mermaid
quadrantChart
    title "台灣資產管理平台定位（X：價格親民度 / Y：整合廣度）"
    x-axis "高價" --> "低價"
    y-axis "整合窄" --> "整合廣"
    quadrant-1 "低價整合廣（本專案目標）"
    quadrant-2 "高價整合廣"
    quadrant-3 "低價整合窄"
    quadrant-4 "高價整合窄"
    "Mint": [0.95, 0.6]
    "Personal Capital": [0.95, 0.8]
    "YNAB": [0.4, 0.4]
    "Monarch Money": [0.5, 0.7]
    "MoneyDJ": [0.95, 0.5]
    "基富通": [0.95, 0.3]
    "Wealth Dashboard": [0.7, 0.9]
```

**差異化定位**：**中價 + 整合廣（台股 + 加密 + 基金 + ETF）+ 純前端 + 繁中** — Mint / Personal Capital 偏美國；YNAB 偏預算；Monarch 英文；MoneyDJ / 基富通偏單一資產；本專案中價 + 整合廣 + 純前端。

### 15.3 預期收益

**保守估計**（M6 達成）
- 1,000 註冊 × 3% 付費 = 30 付費
- 平均月費 NT$150（混合個人 Pro + 加密版）= NT$4,500 MRR
- 年化 = **NT$54K ARR**

**中等估計**（M12 達成）
- 10,000 註冊 × 4% 付費 = 400 付費
- 平均月費 NT$500（含 10% 高資產版）= NT$200,000 MRR
- 年化 = **NT$2.4M ARR**

**樂觀估計**（M18 達成）
- 50,000 註冊 × 5% 付費 = 2,500 付費
- 平均月費 NT$1,500（含 15% 企業財富管理 + API）= NT$3.75M MRR
- 年化 = **NT$45M ARR**

**Unit Economics**
- **CAC**：NT$500（高資產 LINE 群 + 理財部落客口碑）
- **LTV**：NT$500/月 × 平均訂閱 18 個月 = NT$9,000
- **LTV/CAC 比**：18（健康 SaaS 應 ≥3）

### 15.4 商業化評分（0-100，4 維細項）

| 維度 | 分數 | 評估理由 |
|---|---|---|
| **市場規模** | 95 | NT$532.1 億潛在 ARR，500 萬投資戶 + 300 萬加密持有者 |
| **差異化** | 85 | 整合廣度（台股 + 加密 + 基金 + ETF）+ 純前端 + 繁中為獨特賣點 |
| **變現路徑** | 75 | Freemium + 5 個 tier + 企業財富管理完整 |
| **技術可行性** | 70 | Next.js + 多 API 串接都成熟，但即時報價較複雜 |
| **團隊執行力** | 75 | Alan (CTO) + Hermes Agent 已有 SaaS 經驗 |
| **競爭護城河** | 80 | 整合多資產 + 純前端為護城河，模仿需時 |
| **加權平均** | **80** | 🟢 高水平（接近 80） |

**最終商業化評分**：**80 / 100**（高水平 — 整合廣度 + 純前端 + 繁中 + 5 tier 五引擎驅動，需驗證台灣用戶對資產整合平台接受度）
